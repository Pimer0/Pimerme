...
const ShowCasecard: React.FC<ShowCasecardProps> = ({ id, title, sousTitle, data }) => {
    ...
    return ( 
        ...
        transition={{ type: "spring", stiffness: 400, damping: 17 }} className="bg-zinc-900 max-h-fit w-100 rounded-3xl border-zinc-500 border m-10 overflow-hidden">
            ...
            {!isReactionTaped && <BoutonReaction onClick={handleReactionClick} selectedImage={selectedImage} />}
            {isReactionTaped && <BoutonReactionTaped id={id} onReactionSelect={handleReactionSelect} onClose={handleCloseReaction} />}
        </motion.div>
    );
}
